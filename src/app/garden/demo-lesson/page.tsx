...
const DemoLessonPage = () => {
  ...
  const [celebrationData, setCelebrationData] = useState<CourseCompletionData | null>(null)
    ...
    try {
      ...
      if (response.success) {
        ...
        addNotification({
          ...
          message: `ได้รับ ${response.data.garden_progress.garden_xp} Impact Points และ ${response.data.garden_progress.star_seeds} AI Credits`,
          duration: 5000
        })
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold font-serif text-gray-200 flex items-center space-x-2">
                  <BookOpen className="h-6 w-6 text-mint-500" />
                  <span>ทดสอบ Demo</span>
                </h1>
                <p className="text-gray-500 mt-1">ทำ Challenge สำเร็จเพื่อรับ Impact Points</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8 py-8">
        <div className="bg-gray-900 rounded-sm border border-gray-700 p-6 mb-8">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-500">
              เลือก Challenge เพื่อทดสอบระบบ AI Lab Rewards
            </p>
          </div>
          ...
          <div className="bg-gray-800/50 rounded-sm p-4 mb-6">
            <h3 className="font-semibold text-gray-200 mb-2 flex items-center space-x-2">
              ...
              <span>ระบบ AI Lab Integration</span>
            </h3>
            <div className="text-sm text-gray-400 space-y-1">
              <p>• ✅ ทำ challenge สำเร็จ → ได้ Impact Points + AI Credits อัตโนมัติ</p>
              <p>• ✅ ทำ challenge ครบ → ได้โบนัส reward พิเศษ</p>
              <p>• ✅ Impact tracking → ปลดล็อคตาม progress</p>
              <p>• ✅ Real-time lab updates → เห็นผลทันที</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {demoLessons.map((lesson) => (
            <motion.div
              key={lesson.id}
              className={`bg-gray-900 rounded-sm border overflow-hidden transition-all duration-300 ${
                lesson.isCompleted
                  ? 'border-2 border-green-500/30 bg-gray-800/50'
                  : 'border-gray-700'
              }`}
            ...
            >
              <div className="p-6">
                ...
                {lesson.isCompleted ? (
                  <div className="flex items-center justify-center py-3 bg-green-500/10 rounded-sm">
                    <CheckCircle className="h-5 w-5 text-green-500 mr-2" />
                    <span className="text-green-400 font-medium">เรียนจบแล้ว</span>
                  </div>
                ) : (
                  <motion.button
                    onClick={() => simulateWatchLesson(lesson.id, lesson.duration)}
                    disabled={completingLesson === lesson.id || selectedLesson === lesson.id}
                    className="w-full bg-mint-500 hover:opacity-90 disabled:opacity-40 text-white py-3 px-4 rounded-sm font-medium flex items-center justify-center space-x-2 transition-colors"
                    whileTap={{ scale: 0.98 }}
                  >
                ...
                {!lesson.isCompleted && (
                  <div className="mt-4 bg-gray-800/50 rounded-sm p-3">
                    <h4 className="text-sm font-semibold text-gray-200 mb-2 flex items-center space-x-1">
                      <Star className="h-4 w-4" />
                      <span>รางวัลที่จะได้รับ</span>
                    </h4>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">XP: +{20 + lesson.duration}</span>
                      <span className="text-gray-400">⭐ Seeds: +{Math.floor((20 + lesson.duration) * 0.3)}</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="mt-8 bg-gray-800/50 rounded-sm p-6">
          ...
          <div className="text-sm text-gray-400 space-y-2">
            <p>1. 📚 คลิก &ldquo;เริ่มเรียน&rdquo; ในบทเรียนที่ยังไม่จบ</p>
